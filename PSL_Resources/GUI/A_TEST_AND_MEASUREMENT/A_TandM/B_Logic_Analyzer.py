...
from .templates import auto_digitalScopeNoTrig as digitalScopeNoTrig
...
class AppWindow(QtGui.QMainWindow, digitalScopeNoTrig.Ui_MainWindow, utilitiesClass):
    ...
    def capture(self):
        ...
        if self.channelSelection.currentIndex() == 0:  # 1 channel mode
            ...
            if (trmode):
                self.I.start_one_channel_LA(channel=aqchan, channel_mode=aqmode, trigger_channel=trchan,
                                            trigger_mode=trmode + 1)
        # else : self.I.start_one_channel_LA(channel=aqchan,channel_mode=aqmode,trigger_mode=0)
            ...
            self.I.start_two_channel_LA(modes=self.dchan_modes,
                                        chans=[self.C2_ID1.currentText(), self.C2_ID2.currentText()], trigger=True, \
                                        trig_type=TT,
                                        trig_chan=self.C2_ID1.currentText())  # self.C2_trig.currentText())
        ...
        # else : self.I.start_three_channel_LA(modes=self.dchan_modes,trigger_channel=trchan,trigger_mode=0)
            ...
            self.I.start_four_channel_LA(1, self.dtime, self.dchan_modes, edge='rising',
                                         trigger_ID1=True)  # self.FourChanTrig.isChecked())
    ...
    def showData(self):
        ...
        start *= 1e6;
        end *= 1e6  # Convert back to uS
        colnum = 0;
        labels = []
        for a in range(self.active_dchannels):
            startIndex = np.argmin(abs(self.I.dchans[a].timestamps - start));
            endIndex = np.argmin(abs(self.I.dchans[a].timestamps - end))
            data = self.I.dchans[a].timestamps[startIndex:endIndex]
            self.info.setColumn(colnum, data);
            colnum += 1
            self.info.setColumn(colnum, np.diff(data));
            colnum += 1
            labels.append('Time[%d] uS' % (1 + a));
            labels.append('Difference[%d]' % (1 + a));
    ...
    def plotData(self):
        ...
        if (self.active_dchannels > 3):
            ...
            if self.I.dchans[3].plot_length == 1:  # No level changes were detected
                x = self.I.dchans[3].xaxis[0] * 1e-6;
                y = self.I.dchans[3].yaxis[0] + 8
    ...
    def measure_interval(self):
        for a in range(4):
            i = self.timingResults.item(a, 0);
            i.setText('')
            i = self.timingResults.item(a, 1);
            i.setText('')
        i = self.timingResults.item(0, 0);
        i.setText('Timeout')

        t1, t2 = self.I.MeasureMultipleDigitalEdges(self.edge1chan.currentText(), self.edge2chan.currentText(),
                                                    self.edge1edge.currentText(), self.edge2edge.currentText(),
                                                    self.edge1Count.currentIndex() + 1,
                                                    self.edge2Count.currentIndex() + 1, self.timeoutBox.value() / 1000.)
        pos = 0
        if t1 != None:
            for a in t1:
                i = self.timingResults.item(pos, 0);
                i.setText(applySIPrefix(a, 'S'));
                pos += 1
        pos = 0
        if t2 != None:
            for a in t2:
                i = self.timingResults.item(pos, 1);
                i.setText(applySIPrefix(a, 'S'));
                pos += 1
