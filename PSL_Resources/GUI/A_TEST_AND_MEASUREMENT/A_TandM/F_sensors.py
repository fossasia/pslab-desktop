        ...
        if bridge:
            ...
            if bridge.NUMPLOTS:
                ...
                if not self.active_device_counter:
                    ...
                    curves = [self.addCurve(self.plot, '%s[%s]' % (label[:10], bridge.PLOTNAMES[a])) for a in
                              range(bridge.NUMPLOTS)]
                else:
                    if label:
                    ...
                    curves = [self.addCurve(newplt, '%s[%s]' % (label[:10], bridge.PLOTNAMES[a])) for a in
                              range(bridge.NUMPLOTS)]
                ...
                for a in range(bridge.NUMPLOTS):
                    ...
                    action = QtGui.QCheckBox(
                        '%s' % (bridge.PLOTNAMES[a]))  # self.curveMenu.addAction('%s[%d]'%(label[:12],a))
