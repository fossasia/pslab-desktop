...
from .templates import auto_analogScope as analogScope
...
class AppWindow(QtGui.QMainWindow, analogScope.Ui_MainWindow, utilitiesClass):
    def __init__(self, parent=None, **kwargs):
        ...
        self.max_samples_per_channel = [0, self.MAX_SAMPLES / 4, self.MAX_SAMPLES / 4, self.MAX_SAMPLES / 4,
                                        self.MAX_SAMPLES / 4]
        self.liss_win = None
        ...
        self.trigtext = pg.TextItem(html=self.trigger_text('CH1'), anchor=(1.2, 0.5), border='w', fill=(0, 0, 255, 100),
                                    angle=0)
    ...
    def start_capture(self):
        ...
        try:
            ...
            if self.active_channels:
                ...
                # else:
                self.I.configure_trigger(self.trigger_channel, self.triggerChannelName, self.trigger_level,
    ...
    def update(self):
            ...
            if fitsel.currentIndex() < 4:
                ...
                if self.channel_states[fitsel.currentIndex()]:
                    if fitsel.currentText() == 'CH2':
                        ed = self.fitData(self.I.achans[fitsel.currentIndex()].get_xaxis(),
                    ...
                    if len(ed) == 4:
                        ...
                        msg += 'Fit %c:Amp = %0.3fV \tFreq=%0.2fHz \tOffset=%0.3fV \tPhase=%0.1f%c' % (
                        pos + 65, ed[0], ed[1], ed[2], ed[3], 176)
                        msg += dp
                    else:
                        msg += 'Fit Failed'
        ...
        # print (self.fps,'not available',self.active_channels,self.liss_x,self.liss_y)
            ...
            self.curve_lis.clear()
            for a in [self.curve1, self.curve2, self.curve3, self.curve4]:
                if self.channel_states[pos]: a.setData(self.I.achans[pos].get_xaxis() * 1e-6,
                                                       self.I.achans[pos].get_yaxis(), connect='finite')
                pos += 1
        ...
        if self.mousePoint:
            ...
            if index > 0 and index < maxIndex:
                if self.fourierMode:
                    coords = "%.1f FPS, <span style='color: white'>%s</span>: " % (
                    self.fps, applySIPrefix(self.curve1.getData()[0][index], 'Hz'))
                else:
                    coords = "%.1f FPS, <span style='color: white'>%0.1f uS</span>: " % (
                    self.fps, self.I.achans[0].xaxis[index])
                for a in range(4):
    ...
    def setTriggerLevel(self, val):
        if self.trigger_channel == 0:
            self.triggerChannelName = self.chan1remap
        else:
            self.triggerChannelName = 'CH2'
    ...
    def remap_CH0(self, val):
        ...
        self.plot.setYRange(min(R), max(R))
        if val != 'CH1':
            self.CH1GainBox.setEnabled(False)  # remapped. gain does not apply
        else:
            self.CH1GainBox.setEnabled(True)
    ...
    def plot_liss(self):
        ...
        if not (self.channel_states[self.liss_x] and self.channel_states[self.liss_y]):
            QtGui.QMessageBox.about(self, 'Error : Insufficient Data',
                                    'Please enable the selected channels in the oscilloscope')
            return

        self.liss_win = pg.GraphicsWindow(title="Basic plotting examples")
        self.liss_win.setWindowTitle('pyqtgraph example: Plotting')
        self.p1 = self.liss_win.addPlot(title="Lissajous: x : %s vs y : %s" % (lissx, lissy),
                                        x=self.I.achans[self.liss_x].get_yaxis(),
                                        y=self.I.achans[self.liss_y].get_yaxis())
        self.p1.setLabel('left', lissy);
        self.p1.setLabel('bottom', lissx)
