...
class AppWindow(QtGui.QMainWindow, template_transient.Ui_MainWindow, utilitiesClass):
    ...
    def ZeroToFive(self):
        self.I.set_state(SQR1=0);
        time.sleep(0.2)  # DisCharge Circuit
    ...
    def FiveToZero(self):
        ...
        time.sleep(0.2)  # Charge Circuit
        self.I.__capture_fullspeed_hr__('CH1', 5000, self.tg, 'SET_LOW')
        self.CH1Fit.setData([], [])
        self.loop = self.delayedTask(5000 * self.I.timebase * 1e-3 + 10, self.plotData)

    def plotData(self):
        self.x, self.VMID = self.I.__retrieveBufferData__('CH1', self.I.samples,
    ...
    def fit(self):
        ...
        if (start > 0):
            start = int(round(start / self.I.timebase))
        else:
            start = 0
        if (end > 0):
            end = int(round(end / self.I.timebase))
        else:
            end = 0
    ...
    def showData(self):
        self.lognum += 1
        b = self.CParams
        res = 'FIT:\nAmp:%.1fV\tFreq:%.1fHz\tPhase:%.1f\nOffset:%.2fV\tDamping:%.2e' % (
        b[0], 1e6 * abs(b[1]) / (2 * np.pi), b[2] * 180 / np.pi, b[3], b[4])
