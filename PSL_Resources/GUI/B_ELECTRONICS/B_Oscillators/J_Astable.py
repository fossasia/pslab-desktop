...
params = {
    ...
    'name': "Astable\nMultivibrator",
    'hint': '''
	Study a transistor based astable multivibrator circuit.
...
class AppWindow(QtGui.QMainWindow, template_graph_nofft.Ui_MainWindow, utilitiesClass):
    def __init__(self, parent=None, **kwargs):
        ...
        a1 = {'TITLE': 'Analyse CH1', 'FUNC': self.measureFreq1,
              'TOOLTIP': 'Curve fit the trace from CH1 and find the frequency'}
        self.ampGain1 = self.buttonIcon(**a1)
        self.WidgetLayout.addWidget(self.ampGain1)

        a1 = {'TITLE': 'Analyse CH2', 'FUNC': self.measureFreq2,
              'TOOLTIP': 'Curve fit the trace from CH2 and find the frequency'}
    ...
    def measureFreq1(self):
        self.fit1 = True
        return 'measuring..'

    def measureFreq2(self):
        self.fit2 = True
        return 'measuring..'
    ...
    def run(self):
        ...
        try:
            self.I.configure_trigger(0, 'CH1', 1.0, prescaler=self.prescalerValue)
    ...
    def plotData(self):
        ...
        try:
            ...
            if self.fit1:
                ...
                try:
                    ...
                    if fitres:
                        ...
                        self.ampGain1.value.setText('F=%.3f Hz' % (freq))
                    else:
                        self.ampGain1.value.setText('Fit Error')
                except:
                    self.ampGain1.value.setText('Fit Error')
                    pass
            ...
            if self.fit2:
                self.fit2 = False
                try:
                    fitres = self.math.sineFit(self.I.achans[1].get_xaxis(), self.I.achans[1].get_yaxis())
                    if fitres:
                        amp, freq, offset, phase = fitres
                        self.ampGain2.value.setText('F=%.3f Hz' % (freq))
                    else:
                        self.ampGain2.value.setText('Fit Error')
                except:
                    self.ampGain2.value.setText('Fit Error')
                    pass
