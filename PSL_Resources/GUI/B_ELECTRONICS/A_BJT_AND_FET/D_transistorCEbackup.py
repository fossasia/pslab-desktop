...
class AppWindow(QtGui.QMainWindow, transistor.Ui_MainWindow, utilitiesClass):
    def __init__(self, parent=None, **kwargs):
        ...
        self.plotType = 0
        self.optionsBox.addItems(['Output Characteristics (Ice  -  Vce)', 'Input Characteristics (Ice  -  Vbe)',
                                  'Transfer Characteristics (Ibe  -  Ice)'])
    ...
    def typeChanged(self, num):
        ...
        self.biasV.setMinimum(self.biasLimits[num] * -1);
        self.biasV.setMaximum(self.biasLimits[num])
        self.sweepLabel.setText(self.sweepLabels[num])
        self.startV.setMinimum(self.sweepLimits[num] * -1);
        self.startV.setMaximum(self.sweepLimits[num])
        self.stopV.setMinimum(self.sweepLimits[num] * -1);
        self.stopV.setMaximum(self.sweepLimits[num])
    ...
    def acquire(self):
        ...
        if self.V > self.stopV.value():
            self.looptimer.stop()
            txt = '<div style="text-align: center"><span style="color: #FFF;font-size:8pt;">%s</span></div>' % (
                self.traceName)
    ...
    def delete_curve(self):
        ...
        if c > -1:
            ...
            self.curves[self.plotType].pop(c);
            self.curveLabels[self.plotType].pop(c);
