...
class AppWindow(QtGui.QMainWindow, testing.Ui_MainWindow, utilitiesClass):
    ...
    def I2CScan(self, row):
        ...
        if 96 in res:
            self.setSuccess(item, 1)  # DAC found
        else:
    ...
    def SQRID(self, row):
        ...
        try:
            avg = np.average(res)
            item.setText('%.3e' % avg)
            if abs(avg - float(self.tbl.item(row, 0).text())) < 20:
    ...
    def __PVCH__(self, DAC, ADC, row, rng):
        ...
        if np.any(abs(read - actuals) > 10e-3):
    ...
    def calFinished(self, items):
        ...
        reply = QtGui.QMessageBox.question(self, 'Cross Check',
                                           'Does the plot look okay? proceed with writing to flash?',
        ...
        LOOKBEHIND = 100;
        LOOKAHEAD = 100
        OFF = np.array([np.argmin(np.fabs(YDATA[max(B - LOOKBEHIND, 0):min(4095, B + LOOKAHEAD)] - X[B])) - (
    ...
    def PCSCH3(self, row):
        ...
        if np.any(abs(read - actuals) > 10e-6):
            self.setSuccess(self.tbl.item(row, 1), 0)
        else:
            self.setSuccess(self.tbl.item(row, 1), 1)
    ...
    def correct(self):
        ...
        self.scalers[4] *= self.CR0;
        self.scalers[5] *= self.CR1;
        self.scalers[6] *= self.CR2;
        self.scalers[7] *= self.CR3;

        self.displayDialog(
            'loading %s\nPCS SLOPE:%s\nPCS OFFSET:%s\nCR0 : %.3f\nCR1 : %.3f\nCR2 : %.3f\nCR3 : %.3f\nRES : %.3f\n' % (
            self.scalers, self.PCS_SLOPE, self.PCS_OFFSET, self.CR0, self.CR1, self.CR2, self.CR3,
            self.RESISTANCE_SCALING))
        cap_and_pcs = self.I.write_bulk_flash(self.I.CAP_AND_PCS, self.I.__stoa__(
            'READY' + struct.pack('8f', *self.scalers)))  # READY+calibration_string
        self.I.SOCKET_CAPACITANCE = self.scalers[0]
        self.I.__calibrate_ctmu__(self.scalers[4:])
        self.I.DAC.CHANS['PCS'].load_calibration_twopoint(self.scalers[1],
                                                          self.scalers[2])  # Slope and offset for current source
