...
class acquirer():
    ...
    def getAnotherPoint(self):
                        ...
                        for b in range(8):
                            ...
                            if self.I.analogInputSources[a].__conservativeInRange__(val1):
                                v = self.I.analogInputSources[a].calPoly12(
                                    np.average([self.I.__get_raw_average_voltage__(a) for x in range(20)]))
                    ...
                    if self.I.analogInputSources[a].gainEnabled:
                        for b in range(8):
                            ...
                            if self.I.analogInputSources[a].__conservativeInRange__(val2):
                                v = self.I.analogInputSources[a].calPoly12(
                                    np.average([self.I.__get_raw_average_voltage__(a) for x in range(100)]))
                                self.ADC_VALUES[a][b].append(v)
        ...
        if self.vv % 50 == 0:
            self.parent.curves['PV3'].setData(self.ADC24['AIN7'],
                                              np.array(self.DAC_VALS['PV3']) - np.array(self.ADC24['AIN7']))
            self.parent.curves['INL'].setData(self.ADC24['AIN7'],
                                              np.array(self.ADCPIC_INL) - np.array(self.ADC24['AIN7']))
            for a in self.INPUTS:
                if self.I.analogInputSources[a].gainEnabled:
                    for b in range(8):
...
class AppWindow(QtGui.QMainWindow, calibrator.Ui_MainWindow, utilitiesClass):
    ...
    def __init__(self, parent=None, **kwargs):
        ...
        item = self.addLabel('INL', [255, 255, 255]);
        self.curves['INL'].curve.setClickable(True);
        self.curves['INL'].sigClicked.connect(functools.partial(self.selectItem, item))

        self.curves['PV3'] = self.addCurve(self.plot, pen=pg.mkPen([255, 255, 255], width=1), name='PV3')
        item = self.addLabel('PV3', [255, 255, 255]);
        self.curves['PV3'].curve.setClickable(True);
        self.curves['PV3'].sigClicked.connect(functools.partial(self.selectItem, item))
        ...
        for a in self.INPUTS:
            ...
            if self.I.analogInputSources[a].gainEnabled:
                for b in range(8):
                    ...
                    self.curves[a][b].curve.setClickable(True);
                ...
                item = self.addLabel(name, col);
                self.curves[a][0].curve.setClickable(True);
    ...
    def saveData(self):
        ...
        if self.calibrateOnlyADC:  # create ideal dataset for PV1, PV2
            np.savetxt(os.path.join(self.savedir, 'PV1_ERR.csv'),
                       np.column_stack([np.linspace(-5, 5, 4096), np.linspace(-5, 5, 4096)]))
            np.savetxt(os.path.join(self.savedir, 'PV2_ERR.csv'),
                       np.column_stack([np.linspace(-3.3, 3.3, 4096), np.linspace(-3.3, 3.3, 4096)]))
        else:
            np.savetxt(os.path.join(self.savedir, 'PV1_ERR.csv'),
                       np.column_stack([self.A.ADC24['AIN5'], self.A.DAC_VALS['PV1']]))
            np.savetxt(os.path.join(self.savedir, 'PV2_ERR.csv'),
                       np.column_stack([self.A.ADC24['AIN6'], self.A.DAC_VALS['PV2']]))

        np.savetxt(os.path.join(self.savedir, 'PV3_ERR.csv'),
                   np.column_stack([self.A.ADC24['AIN7'], self.A.DAC_VALS['PV3']]))

        np.savetxt(os.path.join(self.savedir, 'CALIB_INL.csv'),
                   np.column_stack([self.A.ADC24['AIN7'], self.A.ADCPIC_INL]))
        for a in self.INPUTS:
            if self.I.analogInputSources[a].gainEnabled:
                for b in range(8):
                    raw = self.A.ADC_VALUES[a][b]
                    np.savetxt(os.path.join(self.savedir, 'CALIB_%s_%dx.csv' % (a, self.I.gain_values[b])),
                               np.column_stack([np.array(self.A.ADC24['AIN6'])[self.A.ADC_ACTUALS[a][b]], raw]))
            else:
                np.savetxt(os.path.join(self.savedir, 'CALIB_%s_%dx.csv' % (a, 1)), np.column_stack(
                    [np.array(self.A.ADC24['AIN6'])[self.A.ADC_ACTUALS[a][0]], self.A.ADC_VALUES[a][0]]))
