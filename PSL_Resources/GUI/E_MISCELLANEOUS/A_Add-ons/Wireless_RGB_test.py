...
class AppWindow(QtGui.QMainWindow):
    def __init__(self, parent=None, **kwargs):
        ...
        self.chanBox.setMinimum(1);
        self.chanBox.setMaximum(127);
        self.chanBox.setValue(10);
        self.chanBox.valueChanged.connect(self.switchChannel)
    ...
    def update(self):
        ...
        for a in self.slds: self.slds[a][1].WS2812B(
            [[int(self.t1[self.pos]), int(self.t2[self.pos]), int(self.t3[self.pos])]])
        if self.pos % 50 == 0:
            for a in self.slds:
                wid = self.slds[a]
