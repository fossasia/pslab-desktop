...
class AppWindow(QtGui.QMainWindow, widget_layout.Ui_MainWindow, utilitiesClass):
    def __init__(self, parent=None, **kwargs):
        ...
        self.WidgetLayout.addWidget(
            self.simpleButtonIcon(TITLE='Clear OLED', FUNC=self.clear, TOOLTIP='Clear the display'))
        self.WidgetLayout.addWidget(
            self.simpleButtonIcon(TITLE='Test OLED', FUNC=self.test, TOOLTIP='Load the CSpark Logo'))
        ...
        self.WidgetLayout.addWidget(self.dialIcon(TITLE='Threshold', FUNC=self.setThreshold, MIN=1, MAX=254,
                                                  TOOLTIP='Set the threshold for making a monochrome image'))
    ...
    def setThreshold(self, val):
        if self.img:
            ...
            image = QtGui.QImage(bw.convert("RGBA").tostring("raw", "RGBA"), bw.size[0], bw.size[1],
