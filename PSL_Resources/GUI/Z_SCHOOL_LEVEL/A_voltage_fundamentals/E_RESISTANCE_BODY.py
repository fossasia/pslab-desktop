...
class AppWindow(QtGui.QMainWindow, widget_layout.Ui_MainWindow, utilitiesClass):
    def __init__(self, parent=None, **kwargs):
        ...
        self.resmeter = self.displayIcon(TITLE="Body's Resistance (PV3 <-> CH3)", UNITS=u"\u03A9",
                                         TOOLTIP='Displays the value of a resistor connected between PV3 and CH3\nThe known input impedance (1MOhm) of CH3, and measured voltage is used.')
    ...
    def run(self):
        ...
        if V < 0:
            ...
            return
        R = 'Voltage(CH3)\t%s\nCurrent Flow\t%s\nResistance\t%s' % (
        self.applySIPrefix(V, 'V'), self.applySIPrefix(V / 1e6, 'A'),
        self.applySIPrefix(1e6 * (3.0 - V) / V, u"\u03A9"))
