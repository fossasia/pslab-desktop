...
class AppWindow(QtGui.QMainWindow, template_graph_nofft.Ui_MainWindow, utilitiesClass):
    def __init__(self, parent=None, **kwargs):
        ...
        self.voltmeter = self.displayIcon(TITLE='CH1 Voltage', UNITS='V',
                                          TOOLTIP='Displays instantaneous voltage on CH1 using the voltmeter')
        self.WidgetLayout.addWidget(self.voltmeter)
        self.voltmeter2 = self.displayIcon(TITLE='CH2 Voltage', UNITS='V',
                                           TOOLTIP='Displays instantaneous voltage on CH1 using the voltmeter')
    ...
    def run(self):
        ...
        try:
            self.I.configure_trigger(0, 'CH1', 0, prescaler=3)
            self.I.capture_traces(2, self.samples, self.tg)
            if self.running: self.timer.singleShot(self.samples * self.I.timebase * 1e-3 + 150, self.plotData)
        except:
            pass
    ...
    def plotData(self):
        ...
        try:
            ...
            self.voltmeter.setValue(self.math.RMS(self.I.achans[0].get_yaxis()))
            self.voltmeter2.setValue(self.math.RMS(self.I.achans[1].get_yaxis()))

            self.displayCrossHairData(self.plot, False, self.samples, self.I.timebase,
    ...
    def crossHairEvent(self, plot, evt):
        ...
        if plot.sceneBoundingRect().contains(pos):
            ...
            self.displayCrossHairData(plot, False, self.samples, self.I.timebase,
                                      [self.I.achans[0].get_yaxis(), self.I.achans[1].get_yaxis()],
                                      [(0, 255, 0), (255, 0, 0)])
