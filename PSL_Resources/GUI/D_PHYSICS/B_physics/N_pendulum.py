...
class AppWindow(QtGui.QMainWindow, auto_simplePendulum.Ui_MainWindow, utilitiesClass):
    ...
    def plotData(self):
        self.x, self.VMID = self.I.__retrieveBufferData__('CH3', self.I.samples,
                                                          self.I.timebase)  # self.I.fetch_trace(1)
    ...
    def fit(self):
        ...
        if (start > 0):
            start = int(round(1.e6 * start / self.I.timebase))
        else:
            start = 0
        if (end > 0):
            end = int(round(1.e6 * end / self.I.timebase))
        else:
            end = 0
        guess = self.CC.getGuessValues(self.x[start:end] - self.x[start], self.VMID[start:end], func='damped sine')
        Csuccess, Cparams, chisq = self.CC.arbitFit(self.x[start:end] - self.x[start], self.VMID[start:end],
                                                    self.CC.dampedSine, guess=guess)

        if Csuccess:
            showVals = [Cparams[0], 1e6 * abs(Cparams[1]) / (2 * np.pi), Cparams[2] * 180 / np.pi, Cparams[3],
                        Cparams[4]]
            for a in range(5):
                item = self.FitTable.item(0, a)
                item.setText('%.2e' % showVals[a])
            self.CH1Fit.setData(self.x[start:end] * 1e-6,
                                self.CC.dampedSine(self.x[start:end] - self.x[start], *Cparams))
