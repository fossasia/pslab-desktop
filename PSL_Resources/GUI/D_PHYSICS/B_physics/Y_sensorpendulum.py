...
class AppWindow(QtGui.QMainWindow, template_transient.Ui_MainWindow, utilitiesClass):
    ...
    def fit(self):
        ...
        if (start > 0):
            start = int(round(1.e6 * start / self.tg))
        else:
            start = 0
        if (end > 0):
            end = int(round(1.e6 * end / self.tg))
        else:
            end = 0
        ...
        for a in range(6):
            ...
            Csuccess, Cparams, chisq = self.CC.arbitFit(DT * 1e-6, self.datasets[a][start:end], self.CC.dampedSine,
                                                        guess=guess)
            if Csuccess:
    ...
    def saveData(self):
        self.saveDataWindow([self.curveGx, self.curveGy, self.curveGz, self.curveAx, self.curveAy, self.curveAz],
                            self.plot1)
